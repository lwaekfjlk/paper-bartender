...
Available commands:
1. add_paper: Add a new paper with name, deadline, and optional pdf_path
2. add_milestone: Add a milestone to a paper with paper_name, description, and due_date
3. add_milestones: Add MULTIPLE milestones to a paper at once. Use this when user mentions multiple tasks/milestones with different due dates.
4. update_paper: Update an existing paper's properties (like pdf_path, deadline, description)
5. unknown: If the input doesn't match any command

Parse the input and return ONLY a JSON object with:
- "command": one of "add_paper", "add_milestone", "add_milestones", "update_paper", "unknown"
...
- "set the PDF path of MyPaper to ~/paper.pdf" -> {{"command": "update_paper", "params": {{"name": "MyPaper", "pdf_path": "~/paper.pdf"}}}}
- "link ~/Downloads/draft.pdf to ML Research" -> {{"command": "update_paper", "params": {{"name": "ML Research", "pdf_path": "~/Downloads/draft.pdf"}}}}
